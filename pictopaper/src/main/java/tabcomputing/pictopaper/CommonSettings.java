...
public class CommonSettings extends AbstractSettings {
    ...
    public void preferenceChanged(String key) {
        ...
        if (key.equals(KEY_TIME_SYSTEM)) {
            timeSystem = getTimeSystem();
        } else if (key.equals(KEY_TYPEFACE)) {
            typeface = getTypeface();
        } else if (key.equals(KEY_COLOR_GAMUT)) {
            //colorWheel = settings.getColorWheel();
            colorWheel.setColorGamut(getColorGamut());
        } else if (key.equals(KEY_COLOR_DAYLIGHT)) {
            //colorWheel = settings.getColorWheel();
            colorWheel.setDaylightFactor(isDaylight() ? 0.7 : 0.0);
        }
    }
